...
public class BedrockBlock : Block{
    ...
    public BedrockBlock(Vector3 position, Chunk chunk)
		:base(BlockType.BEDROCK, BlockMatterState.SOLID,-1,position,chunk){
	}
	...
	public override Vector2 getBlockUvs(Direction side, int position){		
		if(position==0) return new Vector2( 0.3125f, 0.8125f );
		if(position==1) return new Vector2( 0.375f, 0.8125f);
		if(position==2) return new Vector2( 0.3125f, 0.875f );
		if(position==3) return new Vector2( 0.375f, 0.875f );
