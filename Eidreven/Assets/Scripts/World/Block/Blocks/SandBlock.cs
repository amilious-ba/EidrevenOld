...
public class SandBlock  : Block{
   ...
   public SandBlock(Vector3 position, Chunk chunk)
		:base(BlockType.SAND, BlockMatterState.SOLID,3,position,chunk){
	}
	...
	public override Vector2 getBlockUvs(Direction side, int position){		
		if(position==0) return new Vector2(0.125f,0.875f);
		if(position==1) return new Vector2(0.1875f,0.875f);
		if(position==2) return new Vector2(0.125f,0.9375f);
		if(position==3) return new Vector2(0.1875f,0.9375f);
