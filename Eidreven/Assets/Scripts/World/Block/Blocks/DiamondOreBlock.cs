...
public class DiamondOreBlock : Block{
   ...
   public DiamondOreBlock(Vector3 position, Chunk chunk)
		:base(BlockType.DIAMONDORE, BlockMatterState.SOLID,6,position,chunk){
	}
	...
	public override Vector2 getBlockUvs(Direction side, int position){		
		if(position==0) return new Vector2( 0.125f, 0.75f );
		if(position==1) return new Vector2( 0.1875f, 0.75f);
		if(position==2) return new Vector2( 0.125f, 0.8125f );
		if(position==3) return new Vector2( 0.1875f, 0.8125f );
